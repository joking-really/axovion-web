/* ============================================================================
   ...
   FORK NOTES — Axovion.io (this file is a fork of upstream, not upstream)
   --------------------------------------------------------------------------
   Upstream `mountScrollWorld()` returns nothing and never tears down. In a
   react-router SPA that leaks on every navigation away from Home and
   double-binds on the way back. Everything below is additive teardown; the
   mounted behaviour is byte-for-byte the same as upstream.

     1. `mountScrollWorld()` now RETURNS a `destroy()` function. The no-sections
        early return returns a no-op `destroy` so callers never branch on it.
        `destroy()` is idempotent.
     2. The scroll handler was an anonymous inline function and therefore
        unremovable. Hoisted to a named `onScroll`. `resize` / `orientationchange`
        / `load` / `pointerdown` / `touchstart` were already named; all six are
        now removed in `destroy()`.
     3. The rAF loop is captured: `rafId` holds the id from the initial
        `requestAnimationFrame(raf)` and from the re-arm at the end of `raf()`;
        `readRafId` holds the coalesced read frame scheduled by `onScroll`.
        Both are cancelled in `destroy()`, and `raf()` bails immediately once
        `destroyed` is set.
     4. Every blob URL minted from a fetched clip is stored on its segment as
        `s.blobUrl` and passed to `URL.revokeObjectURL` in `destroy()`. The
        video's `src` is nulled and the attribute removed before `load()` so the
        decoder releases the buffer instead of holding it until GC.
     5. Clip fetches run under a single `AbortController` (`fetchAbort`), aborted
        in `destroy()`. A `destroyed` guard inside the `.then` also revokes and
        drops any blob that resolves during the teardown race.
     6. `injectCSS()` is refcounted through the module-scoped `SW_MOUNTS`
        counter. The `#sw-css` style element is only removed when the last
        instance unmounts, so a second concurrently-mounted instance (React
        StrictMode double-invoke, or two worlds on one page) keeps its styles.
     7. `destroy()` empties the container, drops the `sw-root` class and clears
        the `--sw-accent` custom property the engine writes onto the container,
        so a remount starts from a clean node.

   SCROLL LISTENER EXCEPTION
   --------------------------------------------------------------------------
   DESIGN.md hard-bans `window.addEventListener('scroll', …)` in our own code.
   The listener kept here is the sanctioned exception: it is `{ passive: true }`
   and does nothing but coalesce into a single `requestAnimationFrame(read)`,
   which is the correct way to drive a scrubber. IntersectionObserver cannot
   produce a continuous scroll offset, and framer-motion's `useScroll` would
   push a per-frame value through React for a vanilla DOM engine. Leave it.
   ========================================================================== */

// Refcount for the shared `#sw-css` style element. Incremented per mount,
// decremented per destroy; the element is only removed at zero.
let SW_MOUNTS = 0;
...
function mountScrollWorld(container, config) {
  ...
  // ---- teardown bookkeeping (fork addition) ----
  let destroyed = false;
  let rafId = 0;        // the scrub loop
  let readRafId = 0;    // the scroll-coalesced read()
  ...
  SEGMENTS.forEach(s => {
    ...
    s.blobUrl = null;   // fork: tracked so destroy() can revoke it
  });
  ...
  // FORK: absolute document offset of the track. Upstream assumed the track began at
  // document y=0; with anything above it in flow (a sticky navbar) every segment
  // boundary was shifted by that height and the flight finished early.
  let origin = 0;
  ...
  function layout() {
    ...
    // FORK: measure where the track actually starts in the document.
    origin = track.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
    let off = 0;
  ...
  function read() {
    const y = (window.scrollY || window.pageYOffset) - origin;  // FORK: track-relative
  ...
  function raf() {
    if (destroyed) return;   // fork: stop re-arming once torn down
    ...
    rafId = requestAnimationFrame(raf);   // fork: id captured for cancellation
  }
  ...
  // fork: hoisted out of the inline arrow so removeEventListener has a reference.
  // See FORK NOTES / SCROLL LISTENER EXCEPTION at the top of this file.
  ...
  // ---- teardown (fork addition) --------------------------------------------
  // Fully reverses mount: listeners, rAF loop, blob URLs, decoders, in-flight
  // fetches, the shared stylesheet (refcounted) and the container's own DOM.
  function destroy() {
    if (destroyed) return;
    destroyed = true;
    ...
    if (readRafId) cancelAnimationFrame(readRafId);
    rafId = 0; readRafId = 0;
    ...
    SEGMENTS.forEach(s => {
      ...
      if (v) {
        try { v.pause(); } catch (e) {}
        // Null the source and re-run resource selection with no src so the
        // decoder frees its buffers now rather than whenever GC gets round to it.
        v.src = '';
    ...
    SW_MOUNTS = Math.max(0, SW_MOUNTS - 1);
    if (SW_MOUNTS === 0) {
      const st = document.getElementById('sw-css');
  ...
  return destroy;   // fork: callers unmount with this
}
